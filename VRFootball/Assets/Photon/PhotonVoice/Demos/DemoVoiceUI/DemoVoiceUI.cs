...
namespace Photon.Voice.DemoVoiceUI
{
    ...
    public class DemoVoiceUI : MonoBehaviour
    {
        ...
        protected void Update()
        {
            ...
            if (this.voiceConnection.Client.InRoom)
            {
                Dictionary<int, Photon.Realtime.Player>.ValueCollection temp = this.voiceConnection.Client.CurrentRoom.Players.Values;
                if (temp.Count > 1)
                {
                    foreach (Photon.Realtime.Player p in temp)
                    {
